...
.tchip-full{display:block;width:100%;padding:6px 4px;border-radius:5px;font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:11px;letter-spacing:.2px;line-height:1.15;text-align:center;white-space:normal;word-break:break-word;box-sizing:border-box}
...
let state = { winKey:null, prelim:false, teamName:null, teamContext:'stats', teamPrelim:false, playerName:null, playerBackTo:null };
...
function drawGridHtml(draw, roundNums){
  ...
  return `<div class="draw-grid" style="grid-template-columns:repeat(${draw.length},1fr)">${rds}${opps}${vens}</div>`;
}
...
function goPlayer(name, backTo){
  ...
  if(p){
    ...
    document.getElementById('playerDrawWrap').innerHTML = sosTeamDraw ? `
      <div class="sh" style="margin-top:18px"><div class="st" style="font-size:16px">${possessive(sosHit.team.toUpperCase())} FINALS RUN</div><div class="ss">WHO THEY PLAY, HOME &amp; AWAY</div></div>
      ${drawGridHtml(sosTeamDraw, windowRounds(state.winKey))}` : '';
    const note = document.getElementById('playerNote');
    ...
    document.getElementById('playerDrawWrap').innerHTML = manualTeamDraw ? `
      <div class="sh" style="margin-top:18px"><div class="st" style="font-size:16px">${possessive(manual.team.toUpperCase())} FINALS RUN</div><div class="ss">WHO THEY PLAY, HOME &amp; AWAY</div></div>
      ${drawGridHtml(manualTeamDraw, windowRounds(state.winKey))}` : '';
    const note = document.getElementById('playerNote');
    note.style.display='block';
